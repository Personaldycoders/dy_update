...
dycoders.ev.on('connection.update', async (update) => {
    ...
    try {
    let link = "https://whatsapp.com/channel/0029VagpRL811ulLEm4TnO0h";
    let channelId = link.split('https://whatsapp.com/channel/')[1];

    // Ambil metadata channel
    let data = await dycoders.newsletterMetadata("invite", channelId);

    if (data?.followed) {
        console.log('Bot sudah mengikuti channel ini.');
    } else {
        await dycoders.newsletterFollow(data.id);
        console.log('Berhasil mengikuti channel WhatsApp.');
    }
} catch (err) {
    console.error('Gagal follow channel:', err);
