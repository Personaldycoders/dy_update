...
exports.smsg = (dycoders, m, store) => {
  ...
  if (m.key) {
    ...
    m.sender = dycoders.decodeJid(m.fromMe && dycoders.user.id || m.participant || m.key.participant || m.chat || '');
    if (m.isGroup) m.participant = dycoders.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.sender = dycoders.decodeJid(m.msg.contextInfo.participant);
      m.quoted.fromMe = m.quoted.sender === dycoders.decodeJid(dycoders.user.id);
      ...
      m.quoted.delete = () => dycoders.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => dycoders.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => dycoders.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.url) m.download = () => dycoders.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? dycoders.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : dycoders.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => dycoders.copyNForward(jid, m, forceForward, options);
  dycoders.appenTextMessage = async (text, chatUpdate) => {
    let messages = await generateWAMessage(m.chat, {
      text: text,
      mentions: m.mentionedJid
    }, {
      userJid: dycoders.user.id,
      quoted: m.quoted && m.quoted.fakeObj
    });
    messages.key.fromMe = areJidsSameUser(m.sender, dycoders.user.id);
    ...
    dycoders.ev.emit('messages.upsert', msg);
  };
  return m;
};
